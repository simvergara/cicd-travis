...
describe('Tutorial part 6', () => {
  ...
  describe('Heroes tests', () => {
    ...
    it(`deconstes ${newHeroName} from Heroes list`, async () => {
    ...
    it(`adds back ${targetHero.name}`, async () => {
      const newwHeroName = 'Alice';
      const heroesBefore = await toHeroArray(getPageElts().allHeroes);
      const numHeroes = heroesBefore.length;

      element(by.css('input')).sendKeys(newwHeroName);
      ...
      expect(heroesAfter[numHeroes]).toEqual({id: maxId + 1, name: newwHeroName});
    });
